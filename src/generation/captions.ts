...
export async function generatePost(slot: ContentSlot, hints?: PlanHints): Promise<GeneratedPost> {
  ...
  if (slot.format === "carousel") {
    const prompt = [
      ...
      "   step-by-step idea better than a photo story. Each imagePrompt still describes a real scene/visual",
      "   consistent with the visual direction below — NOT a plain texture/gradient — and that scene must be",
      "   genuinely RELATED to what the text says (e.g. a 'signs your date nights need a reset' slide shows a",
      "   visually relevant moment, not a random unrelated image). Place the text in a clear zone that does NOT",
      "   cover or obscure the main visual — e.g. a solid-color banner strip along the top or bottom, or a",
      "   negative-space area of the composition — describe the exact placement in the imagePrompt. ONE short,",
      "   large, perfectly legible line of text per slide (a tip, a step, a fact) — this is the ONE exception",
      "   to the 'no text in images' rule above. Keep the text genuinely short (under ~8 words) so an image",
      "   model can render it legibly; describe the exact wording in the imagePrompt. The on-image text is",
      ...
      "   REALISM_DIRECTIVE below still applies to the underlying scene/visual (not to the text itself) —",
      "   informational slides now use a real scene too, so keep it looking like a real photograph.",
      "",
      INFORMATIONAL_CAROUSEL_TEMPLATES,
      "(Only relevant if you chose the INFORMATIONAL style above.)",
      "",
      REALISM_DIRECTIVE,
      "Apply this to EVERY slide's imagePrompt individually, not just the overall series — for NARRATIVE",
      "slides this governs the whole image; for INFORMATIONAL slides it governs the underlying scene/visual",
      "only, not the text overlay.",
    ].join("\n");
  ...
  const prompt = [
    ...
    "",
    "Choose ONE of two reel styles, whichever better fits tonight's theme:",
    "1. NARRATIVE (mood) — the default: a single evocative scene per the visual direction above, following",
    "   the hook-tension-payoff arc.",
    "2. INSTRUCTIONAL (mistake/comparison) — use this when the theme suits a quick 'common mistake', 'right",
    "   way vs wrong way', or 'before vs after' idea better than a mood scene. Carry the actual lesson/",
    "   comparison in the CAPTION (e.g. 'This is what's quietly killing your date nights. This is the fix.') —",
    "   NEVER via on-screen text or UI, video models cannot render legible on-screen text reliably. videoPrompt",
    "   describes ONLY ONE clean visual contrast beat (e.g. a distracted/distant posture shifting to a close/",
    "   present one, or a lighting/color shift) — an ~8s clip has no room for more than one contrast.",
    "",
    REALISM_DIRECTIVE,
    "Apply this to whichever style you chose above — both still need to look like a real photograph/video.",
